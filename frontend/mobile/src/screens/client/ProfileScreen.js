  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.background }} contentContainerStyle={styles.container}>
      ...
      <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
        <Row label="Modo atual" value="Cliente 📱" theme={theme} />
        <Row label="Pontos" value={`${user?.points || 0} pts ⭐`} theme={theme} />
      </View>

      <Pressable
        style={[styles.btn, { borderColor: theme.workerPrimary }]}
        onPress={() => switchMode('worker')}
      >
        <Text style={[styles.btnText, { color: theme.workerPrimary }]}>🔧 Virar Trabalhador</Text>
      </Pressable>
